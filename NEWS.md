...
- Re-implemented `Create.actor.twitter` and `Create.activity.twitter` to use `dplyr` and `data.table` techniques consistent with other package network creation functions. Both functions are significantly faster for large collection datasets.

## Minor Changes
- `Create.actor.twitter` includes two new parameters for `mentions`, `inclMentions` that will process and include `mentions` edges in the network and `inclRtMentions` that will process and include mentions found in retweets. Both parameters are set to `FALSE` by default, and `inclRtMentions` requires that `inclMentions` also be `TRUE`.
