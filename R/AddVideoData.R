...
#' @param actorSubOnly Logical. Only substitute video ID's for their publishers channel ID.
#' Don't add additional video data to edge list.
#' @param ... Additional parameters passed to function.
#'
#' @return Network as a named list of three dataframes containing \code{$nodes}, \code{$edges}
#' and \code{$videos} nodes and edges include columns for additional video data.
...
#' @examples
#' \dontrun{
#' # add video data to an actor network
...
# when ID's are supplied API request does not support paging
