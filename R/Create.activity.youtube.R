...
Create.activity.youtube <- function(datasource, type, verbose = TRUE, ...) {
  ...
  for (row in 1:nrow(node_summary)) {
    ...
    if (type == "comment") df_stats <- network_stats(df_stats, "top-level comments", node_summary[row, "num"])
    else if (type == "reply-comment") df_stats <- network_stats(df_stats, "reply comments", node_summary[row, "num"])
    else if (type == "video") df_stats <- network_stats(df_stats, "videos", node_summary[row, "num"])
  }
