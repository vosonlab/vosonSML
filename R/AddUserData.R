...
AddUserData.actor.twitter <-
  function(net,
    ...
    if (lookupUsers) {
      df_users <- lookup_users(net$missing_users, auth = twitterAuth, retryOnRateLimit, ...) |>
...
AddUserData.twomode.twitter <-
  function(net,
    ...
    if (lookupUsers) {
      df_users <- lookup_users(net$missing_users, auth = twitterAuth, retryOnRateLimit, by = "screen_name", ...) |>
        dplyr::rename(user_id = .data$id_str)
...
add_twitter_users <- function(objs, data, rm, by = "user_id") {
  ...
  actor_ids <- objs |>
    dplyr::select(.data$user_id, dplyr::starts_with("screen_name"))
  ...
  if (by == "screen_name") {
    ...
    objs <- objs |>
      dplyr::left_join(all_users, by = c("user_id" = "u.user_id"), na_matches = "never")
  }
...
twitter_user_transforms <- function(x, rm_misc = TRUE) {
  ...
  x <- x |>
    ...
    dplyr::select(-dplyr::starts_with("id_str"))
...
lookup_users <- function(x, auth, retryonratelimit = TRUE, by = "user_id", ...) {
  prompt_and_stop("rtweet", "lookupUsers")

  df_lookup_data <-
    rtweet::lookup_users(
      ifelse(by == "screen_name", x$screen_name, x$user_id),
      parse = TRUE,
      token = auth$auth,
