...
Create.semantic.twitter <- function(datasource, type, removeTermsOrHashtags = NULL,
  ...
  if (verbose) { df_stats <- network_stats(df_stats, "tokens", nrow(tokens_df), FALSE) }
  ...
  if (!is.null(removeTermsOrHashtags) && length(removeTermsOrHashtags) > 0) {
    ...
    if (verbose) {
      df_stats <- network_stats(df_stats, "removed specified", token_count - nrow(tokens_df), FALSE)
    }
  }
  ...
  if (verbose) {
    if ("user" %in% names(type_tally)) {
      df_stats <- network_stats(df_stats, "removed users", type_tally[["user"]], FALSE)
    }
    if ("hashtag" %in% names(type_tally)) {
      df_stats <- network_stats(df_stats, "hashtag count", type_tally[["hashtag"]], FALSE)
    }
    df_stats <- network_stats(df_stats, "unique hashtags", unique_hashtags, FALSE)
  }
  ...
  rm_words <- function(rm_type, keep_perc) {
    ...
    if (nrow(type_df) > 0) {
      ...
      if (n_value <= 1 & keep_perc != 100) {
        ...
        if (verbose) {
          df_stats <<- network_stats(df_stats, paste0("top ", keep_perc , "% ", rm_type, "s"), keep_count, FALSE)
        }
      } else {
        ...
        if (verbose) {
          df_stats <<- network_stats(df_stats, paste0("top ", keep_perc , "% ", rm_type, "s (freq>=", n_value, ")"),
  ...
  if (verbose) {
    if ("term" %in% names(type_tally)) { df_stats <- network_stats(df_stats, "term count", type_tally[["term"]], FALSE) }

    unique_terms <- nrow(dplyr::distinct(freq_df %>% dplyr::filter(type == "term"), .data$word))
    df_stats <- network_stats(df_stats, "unique terms", unique_terms, FALSE)
  }
