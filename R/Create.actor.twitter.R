...
Create.actor.twitter <-
  function(datasource, type, verbose = TRUE, ...) {
    ...
    is_retweet <-
      is_quote <- mentions_user_id <- reply_to_user_id <- NULL
    ...
    df_stats <- network_stats(NULL, "collected tweets", nrow(df))

    df_users <-
      data.frame("user_id" = character(0), "screen_name" = character(0))
    df_users <-
      rbind(df_users, subset(
        df,
    ...
    dt_combined <- data.table(
      from = as.character(c(rep(
        "NA_f00", 20000000
      ))),
      to = as.character(c(rep(
        "NA_f00", 20000000
      ))),
      edge_type = as.character(c(rep(
        "NA_f00", 20000000
      ))),
      # edgeType
      timestamp = as.character(c(rep(
        "NA_f00", 20000000
      ))),
      # timeStamp
      status_id = as.character(c(rep(
        "NA_f00", 20000000
      ))) # tweet_id
    )
    ...
    nextEmptyRow <-
      1 # so we can update rows in dt_combined in a relatively efficient way
    ...
    count <- 0
    for (i in 1:nrow(df)) {
      if ((df[i, is_retweet][[1]] == FALSE) ||
          (is.na(df[i, is_retweet][[1]]))) {
        next
      }
      ...
      df_users <-
        rbind(df_users,
              list(df$retweet_user_id[i][[1]], df$retweet_screen_name[i][[1]]))

      nextEmptyRow <- nextEmptyRow + 1
    }
    df_stats <- network_stats(df_stats, "retweets", count, TRUE)
    ...
    count <- 0
    for (i in 1:nrow(df)) {
      if ((df[i, is_quote][[1]] == FALSE) ||
          (is.na(df[i, is_quote][[1]]))) {
        next
      }
      ...
      df_users <-
        rbind(df_users,
              list(df$quoted_user_id[i][[1]], df$quoted_screen_name[i][[1]]))

      nextEmptyRow <- nextEmptyRow + 1
    }
    df_stats <-
      network_stats(df_stats, "quoting others", count, TRUE)
    ...
    for (i in 1:nrow(df)) {
      if ((length(df[i, mentions_user_id][[1]]) < 1) |
          (length(df[i, mentions_user_id][[1]]) == 1 &
           is.na(df[i, mentions_user_id][[1]][[1]])) |
          (if_retweet_inlude_mentions == FALSE &
      ...
      for (j in 1:length(df$mentions_user_id[i][[1]])) {
        # for each row of the likes data for post i
        ...
        df_users <-
          rbind(df_users,
                list(df$mentions_user_id[i][[1]][j], df$mentions_screen_name[i][[1]][j]))

        nextEmptyRow <- nextEmptyRow + 1
      }
    }
    df_stats <- network_stats(df_stats, "mentions", mcount, TRUE)
    df_stats <-
      network_stats(df_stats, "reply mentions", rmcount, TRUE)
    ...
    for (i in 1:nrow(df)) {
      ...
      df_users <-
        rbind(df_users,
              list(df$reply_to_user_id[i][[1]], df$reply_to_screen_name[i][[1]]))

      nextEmptyRow <-
        nextEmptyRow + 1 # increment the row to update in dt_combined
    }
    df_stats <- network_stats(df_stats, "replies", count, TRUE)
    ...
    for (i in 1:nrow(df)) {
      ...
      df_users <-
        rbind(df_users, list(df$user_id[i][[1]], df$screen_name[i][[1]]))

      nextEmptyRow <-
        nextEmptyRow + 1 # increment the row to update in dt_combined
    }
    df_stats <- network_stats(df_stats, "self-loops", count, TRUE)

    dt_combined <- dt_combined[edge_type != "NA_f00"]

    # make a vector of all the unique actors in the network
    # df_users <- unique(df_users)
    df_users %<>% dplyr::distinct(.data$user_id, .keep_all = TRUE)

    df_stats <- network_stats(df_stats, "nodes", nrow(df_users))
    df_stats <-
      network_stats(df_stats, "edges", sum(df_stats$count[df_stats$edge_count == TRUE]))
    ...
    class(func_output) <-
      append(class(func_output), c("network", "actor", "twitter"))
    cat("Done.\n")
