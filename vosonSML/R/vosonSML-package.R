...
#' (extension of class \code{data.frame}) that are assigned the class \code{datasource}. Additionally,
#' \code{datasource} objects are attributed a class identifying the source of data, e.g. \code{twitter} or
#' \code{youtube}. In this way \code{datasource} objects are fast, easy to work with, and can be used as input to
#' easily construct different kinds of networks. For example, the function \code{Collect} can be used to collect
#' twitter data, which is then passed to the \code{Create} function resulting in a twitter network (as igraph object)
