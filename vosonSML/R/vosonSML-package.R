...
#' sources (Instagram, Facebook, Twitter, and Youtube) and generating different types of networks suited to
#' Social Network Analysis (SNA) and text analytics. It offers tools to create unimodal, multimodal, semantic, and 
#' dynamic networks. It draws on excellent packages such as \pkg{twitteR}, \pkg{instaR}, \pkg{Rfacebook}, and
#' \pkg{igraph} in order to provide an integrated 'work flow' for collecting different types of social media data and 
#' creating different types of networks out of these data. Creating networks from social media data is often 
#' non-trivial and time consuming. This package simplifies such tasks so users can focus on analysis.
#'
...
#' easily construct different types of networks. For example, the function \code{\link{Collect}} can be used to collect 
#' Twitter data, which is then 'piped' to the \code{\link{Create}} function, resulting in a network (an igraph object) 
#' that is ready for analysis.
#'
#' @name vosonSML-package
#' @aliases vosonSML-package vosonSML
#' @docType package
#' @author Timothy Graham & Robert Ackland, with contribution Chung-hong Chan & Bryan Gertzel
#'
