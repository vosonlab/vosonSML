...
#'
#' This function is used to 'populate' Twitter networks (generated with the \code{\link{Create}} function) with 
#' information about the users in the network. This involves calls to the Twitter API to collect this information, 
#' which is then applied to the network as vertex attributes.
#'
#' @param networkObject An igraph graph object created with \code{\link{Create}}.
#' 
#' @return An igraph graph object with additional user information.
#' 
...
#' myTwitNetwork_UserInfo <- PopulateUserInfoTwitter(myTwitNetwork)
#' }
#' 
#' @export
PopulateUserInfoTwitter <- function(networkObject) {
  name = NULL # appease the gods of R CMD CHECK
  
  # perform the lookups in batches and mind the rate limit
  getUserObjects <- function(users) {
    ...
    userObjects <- ldply(groups, function(group) { # ldply is a very cool function, found in plyr package
      ...
      # twListToDF is also a handy function, found in twitteR package. converts weird class object to data frame
      out <- twListToDF(objects) 
      
      # print("Waiting for 15 minutes (to 'refresh' the rate limit)...") # Don't need to use this yet. 
      # implement later for number of users > 7500 (have to do chunked batches..chunks of chunks..urrghh)
      # Sys.sleep(900)
      return(out)
    })
  ...
  if (length(listOfUsers_actual) < 1) {
    ...
    # this function is supposed to perform the lookups in batches and mind the rate limit
    getUserObjects <- function(users) {
      groups <- split(users, ceiling(seq_along(users) / 50))
      userObjects <- ldply(groups, function(group) { # ldply is a very cool function, found in plyr package.
        objects <- lookupUsers(group, includeNA = TRUE)
